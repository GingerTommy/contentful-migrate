...
const bootstrap = async (
  ...
  try {
    ...
    const files = await generateScripts(spaceId, environmentId, contentTypes, accessToken, httpsAgent, migrationsDirectory)
    if (writeMigrationState) {
      rewriteMigration(spaceId, environmentId, accessToken, httpsAgent, files)
    }
